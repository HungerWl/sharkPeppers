  ...
  <template v-for="route in props.routes" :key="route.path">
    <el-sub-menu v-if="route.children" :index="route.path">
      <template #title>
        ...
        <span v-if="route.meta?.title" style="text-overflow: ellipsis;overflow: hidden;" class="text-[16px]">
          {{ $t(route.meta.title) }}
        </span>
      </template>
    ...
    <el-menu-item v-else :index="isLink(route.path) ? '' : route.path" @click="openLink(route.path)">
      ...
      <template v-if="route.meta?.title" #title>
        <span style="text-overflow: ellipsis;overflow: hidden;" class="text-[16px]">
          {{ $t(route.meta.title) }}
        </span>
      </template>
    </el-menu-item>
